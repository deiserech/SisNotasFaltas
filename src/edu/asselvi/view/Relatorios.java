...
public class Relatorios {
	...
	public static void relatorioNotas(Turma turma, Map<Integer, Aluno> alunos, Map<Integer, Disciplina> disciplinas,
			List<Nota> notas, List<Integer> discProf) {
		...
		if (notas.size() > 0) {
			for (Nota nota : notas) {
				if (discProf.contains(nota.getDisciplinaId())) {
					System.out.println(
							"Bimestre: " + nota.getBimestreId() + "\tAluno: " + alunos.get(nota.getAlunoId()).getNome()
	...
	public static void relatorioFrequencia(Turma turma, Map<Integer, Aluno> alunos,
			Map<Integer, Disciplina> disciplinas, List<Frequencia> frequencias) {
		System.out.println("");
		System.out.println("----------------------------------");
		...
		if (frequencias.size() > 0) {
			for (Frequencia freq : frequencias) {
				System.out.println("Bimestre: " + freq.getBimestreId() + "\tAluno: "
						+ alunos.get(freq.getAlunoId()).getNome() + "\t\tData: " + sdf.format(freq.getDataAula())
